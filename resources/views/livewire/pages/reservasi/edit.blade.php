...
mount(function() {
    $reservasiId = request()->route('reservasi');
    $reservasi = Reservasi::findOrFail($reservasiId);
    $this->reservasiId = $reservasi->id;
    $this->kode_reservasi = $reservasi->kode_reservasi;
    $this->kamar_id = $reservasi->kamar_id;
    $this->pelanggan_id = $reservasi->pelanggan_id;
    $this->tipe_paket = $reservasi->tipe_paket ?? 'harian';
    $this->durasi = $reservasi->durasi ?? 1;
    ...
    // Set unit label berdasarkan tipe paket
    ...
    // Set harga per unit dan hitung total harga
    if ($this->kamar_id) {
    ...
    // Hitung total harga dan tanggal checkout setelah semua data di-load
    if ($this->kamar_id && $this->durasi > 0) {
        $this->hitungTotalHarga();
        if ($this->tanggal_check_in) {
            $this->hitungTanggalCheckout();
        }
    }

    \Log::info('Final state after mount', [
...
$hitungTanggalCheckout = function() {
    if ($this->tanggal_check_in && $this->durasi > 0) {
        // Buat copy dari tanggal check-in untuk menghindari modifikasi object asli
        ...
        \Log::info('Tanggal checkout calculated', [
            'tanggal_check_in' => $this->tanggal_check_in,
            'tipe_paket' => $this->tipe_paket,
            'durasi' => $this->durasi,
            'tanggal_check_out' => $this->tanggal_check_out
...
$hitungTotalHarga = function() {
    ...
    if ($this->kamar_id && $this->durasi > 0) {
        ...
        \Log::warning('Data tidak lengkap untuk perhitungan harga', [
            'kamar_id' => $this->kamar_id,
            'durasi' => $this->durasi
        ]);
        $this->harga_per_unit = 0;
...
$validasiOverlap = function() {
    if ($this->kamar_id && $this->tanggal_check_in && $this->tanggal_check_out) {
        $overlap = \App\Models\Reservasi::where('kamar_id', $this->kamar_id)
            ->where('id', '!=', $this->reservasiId) // Exclude current reservasi
...
$save = function() {
    ...
    $this->validate([
        'kode_reservasi' => 'required|string|unique:reservasis,kode_reservasi,' . $this->reservasiId,
    ...
    try {
        $reservasi = Reservasi::findOrFail($this->reservasiId);
        $reservasi->update([
        ...
        \Log::error('Error updating reservasi:', ['error' => $e->getMessage(), 'trace' => $e->getTraceAsString()]);
        session()->flash('error', 'Gagal mengupdate reservasi: ' . $e->getMessage());
    }
};
                                ...
                                <div class="flex items-center gap-4 mb-3">
                                    ...
                                    <div>
                                        ...
                                        <p class="mt-2 text-sm text-gray-600 dark:text-gray-300 font-medium">
                                            Edit reservasi untuk sistem Pondok Putri
                                        </p>
                                    </div>
                                </div>
                            </div>
                    ...
                    <form wire:submit="save" class="p-8 space-y-8">
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                                ...
                                <div class="col-span-3">
                                    ...
                                    <input wire:model="tanggal_check_out" type="date" id="tanggal_check_out"
                                        class="block w-full px-4 py-3 border-0 rounded-2xl bg-gray-50/80 dark:bg-gray-700/80 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:outline-none backdrop-blur-sm text-sm font-medium shadow-lg"
                                        readonly>
                                    <p class="mt-2 text-xs text-gray-500 dark:text-gray-400">
                        ...
                        <div
                            ...
                            <button type="submit"
                                ...
                                <span class="relative z-10">Update Reservasi</span>
                            </button>
                        </div>
                    </form>
                </div>
