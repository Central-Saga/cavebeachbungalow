...
$viewDetails = function ($facilityId) {
    try {
        $facility = FasilitasKamar::with(['tipeKamars:id,nama_tipe'])->findOrFail($facilityId);

        $this->selectedFacility = [
            'id' => $facility->id,
            'nama_fasilitas' => $facility->nama_fasilitas,
            'created_at' => optional($facility->created_at)->format('d M Y'),
            'tipe_count' => $facility->tipeKamars->count(),
            'tipe_list' => $facility->tipeKamars->pluck('nama_tipe')->sort()->values()->toArray(),
        ];
        $this->showDetailModal = true;
    } catch (\Exception $e) {
        \Log::error('Gagal load detail fasilitas', ['error' => $e->getMessage()]);
        session()->flash('error', 'Fasilitas tidak ditemukan.');
    }
};

$closeDetails = fn () => ($this->showDetailModal = false) && ($this->selectedFacility = null);

$confirmDeleteFacility = function ($facilityId) {
    try {
        $facility = FasilitasKamar::findOrFail($facilityId);
...
$deleteFacility = function ($facilityId) {
    try {
        $facility = FasilitasKamar::findOrFail($facilityId);
        // Lepas relasi pivot agar bersih (jaga-jaga bila tidak cascade)
        $facility->tipeKamars()->detach();
        $facility->delete();
...
$cancelDelete = fn () => ($this->showDeleteModal = false) && ($this->facilityToDelete = null);

with(function () {
    try {
        $query = FasilitasKamar::query()->withCount('tipeKamars');
        ...
        $sortable = ['nama_fasilitas', 'tipe_kamars_count', 'created_at'];
        $sortField = in_array($this->sortField, $sortable) ? $this->sortField : 'created_at';
        $sortDirection = $this->sortDirection === 'asc' ? 'asc' : 'desc';

        $facilities = $query->orderBy($sortField, $sortDirection)->paginate(10);

        // Statistik global
        $globalStats = [
            'totalFacilities' => FasilitasKamar::count(),
            'usedFacilities' => FasilitasKamar::has('tipeKamars')->count(),
            'unusedFacilities' => FasilitasKamar::doesntHave('tipeKamars')->count(),
            'totalTypes' => TipeKamar::count(),
        ];

        \Log::info('Facilities data loaded', [
            'facilities_count' => $facilities->count(),
            'total_facilities' => $globalStats['totalFacilities'],
            'search' => $this->search,
        ...
        \Log::error('Error loading facilities data', [
            'error' => $e->getMessage(),
...
<div
                            ...
                            <div class="flex-1">
                                <div class="flex items-center gap-4 mb-3">
                                    <div
                                        ...
                                        <flux:icon name="adjustments-horizontal" class="w-8 h-8 text-white" />
                                    </div>
                                    <div>
                                        <h1
                                            ...
                                            Kelola Fasilitas Kamar</h1>
                                        <p class="mt-2 text-sm text-gray-600 dark:text-gray-300 font-medium">Master data
                                            fasilitas & keterkaitan dengan tipe kamar</p>
                                    </div>
                                </div>
                            </div>
                            <a href="{{ route('admin.fasilitas-kamar.create') }}"
                                ...
                                <span class="relative z-10">Tambah Fasilitas</span>
                            </a>
                        </div>
                    </div>
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
                ...
                <div
                    ...
                    <div class="relative z-10">
                        ...
                        <h3 class="text-xl font-bold mb-2">{{ $stats['usedFacilities'] }}</h3>
                        <p class="text-emerald-100 text-xs font-medium">Dipakai oleh ≥1 Tipe</p>
                    </div>
                </div>
                ...
                <div
                    ...
                    <div class="relative z-10">
                        ...
                        <h3 class="text-xl font-bold mb-2">{{ $stats['unusedFacilities'] }}</h3>
                        <p class="text-amber-100 text-xs font-medium">Belum Dipakai</p>
                    </div>
                </div>
                ...
                <div
                    ...
                    <div class="relative z-10">
                        ...
                        <p class="text-indigo-100 text-xs font-medium">Total Tipe Kamar</p>
                    </div>
                </div>
            </div>
                        ...
                        <div class="flex flex-col lg:flex-row items-start lg:items-center gap-6">
                            <div class="flex-grow">
                                <div class="relative">
                                    ...
                                    <input wire:model.live="search" type="text" placeholder="Cari fasilitas..."
                            ...
                            <div class="flex gap-3">
                                <button wire:click="sortBy('nama_fasilitas')"
                                    ...
                                    @if($sortField === 'nama_fasilitas')
                                    <span class="ml-2 text-sm">@if($sortDirection === 'asc') ↑ @else ↓ @endif</span>
                                    @endif
                                </button>
                                <button wire:click="sortBy('tipe_kamars_count')"
                                    ...
                                    @if($sortField === 'tipe_kamars_count')
                                    <span class="ml-2 text-sm">@if($sortDirection === 'asc') ↑ @else ↓ @endif</span>
                                    @endif
                                </button>
                            ...
                            <tbody
                                ...
                                <tr
                                            ...
                                            <div class="flex-shrink-0 h-12 w-12">
                                                <div
                                                    ...
                                                    <span class="text-white font-bold text-lg">{{
                                                        strtoupper(substr($f->nama_fasilitas, 0, 1)) }}</span>
                                                </div>
                                            </div>
                                            <div class="ml-5">
                                                <div class="text-sm font-bold text-gray-900 dark:text-white capitalize">
                                                    {{ $f->nama_fasilitas }}</div>
                                                <div class="text-xs text-gray-500 dark:text-gray-400">ID: {{ $f->id }}
                                                </div>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-8 py-6 whitespace-nowrap text-right text-sm font-medium">
                                        <div class="flex space-x-3">
                                            ...
                                            <a href="{{ route('admin.fasilitas-kamar.edit', $f) }}"
                                            ...
                                            <button wire:click="confirmDeleteFacility({{ $f->id }})"
                                ...
                                <tr>
                                    <td colspan="4" class="px-8 py-16 text-center">
                                        ...
                                        <h3 class="text-lg font-bold text-gray-900 dark:text-white mb-3">Belum ada
                                            fasilitas</h3>
                                        <p class="text-sm text-gray-600 dark:text-gray-400 mb-8">Mulai dengan
                                            menambahkan fasilitas baru.</p>
                                        <a href="{{ route('admin.fasilitas-kamar.create') }}"
            ...
            <div
                ...
                <div class="sm:flex sm:items-start">
                    ...
                    <div class="mt-4 text-center sm:ml-6 sm:mt-0 sm:text-left">
                        <h3 class="text-lg font-bold leading-6 text-gray-900 dark:text-white">Hapus Fasilitas</h3>
                        <div class="mt-3">
                            <p class="text-sm text-gray-600 dark:text-gray-300">
                                @if($facilityToDelete)
                                Apakah Anda yakin ingin menghapus fasilitas <strong
                                    class="text-gray-900 dark:text-white font-bold">{{ $facilityToDelete->nama_fasilitas
                                    }}</strong> (ID: {{ $facilityToDelete->id }})?
                                <br><br>
                ...
                <div class="mt-6 sm:mt-5 sm:flex sm:flex-row-reverse gap-4">
                    <button wire:click="deleteFacility({{ $facilityToDelete ? $facilityToDelete->id : 0 }})"
                        type="button"
    ...
    @if($showDetailModal && $selectedFacility)
    <div class="fixed inset-0 z-50 overflow-y-auto">
        <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
            ...
            <div
                    ...
                    <div
                        ...
                        <div
                            ...
                            <span class="text-white font-bold text-base">{{
                                strtoupper(substr($selectedFacility['nama_fasilitas'] ?? 'F', 0, 1)) }}</span>
                        </div>
                    </div>
                    <div class="mt-4 text-center sm:ml-6 sm:mt-0 sm:text-left">
                        <h3 class="text-lg font-bold leading-6 text-gray-900 dark:text-white">Detail Fasilitas</h3>
                        <div class="mt-3 text-sm text-gray-600 dark:text-gray-300">
                            <div class="font-semibold text-gray-900 dark:text-white">{{
                                $selectedFacility['nama_fasilitas'] }}</div>
                            <div class="text-xs mt-1">Dibuat: {{ $selectedFacility['created_at'] }}</div>
                        </div>
                    </div>
                </div>
                ...
                <div class="mt-6 grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <div
                        ...
                        <div class="text-xs font-bold uppercase text-gray-500 dark:text-gray-400 mb-2">Dipakai oleh
                        </div>
                        <div class="flex flex-wrap gap-2 max-h-48 overflow-auto pr-1">
                            @php $tipeList = $selectedFacility['tipe_list'] ?? []; @endphp
                            @forelse($tipeList as $t)
                            <span
                                ...
                                $t }}</span>
                            @empty
                            <span class="text-xs text-gray-500 dark:text-gray-400">Tidak terhubung ke tipe kamar
                                manapun.</span>
                            @endforelse
                        </div>
                    </div>
                    <div
                        ...
                        <div class="text-sm text-gray-700 dark:text-gray-200">
                            <div class="flex items-center justify-between py-1">
                                <span>Tipe terkait</span>
                                <span class="font-bold">{{ $selectedFacility['tipe_count'] }}</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="mt-6 sm:mt-5 sm:flex sm:flex-row-reverse gap-4">
                    <a href="{{ isset($selectedFacility['id']) ? route('admin.fasilitas-kamar.edit', $selectedFacility['id']) : '#' }}"
