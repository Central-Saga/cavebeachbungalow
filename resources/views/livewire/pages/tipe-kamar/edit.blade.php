...
mount(function(TipeKamar $tipeKamar) {
    $this->tipeKamar = $tipeKamar;
    $this->nama_tipe = $tipeKamar->nama_tipe;
    $this->deskripsi = $tipeKamar->deskripsi;
    $this->selectedFasilitas = $tipeKamar->fasilitasKamars->pluck('id')->toArray();
    $this->created_at = $tipeKamar->created_at;
    $this->updated_at = $tipeKamar->updated_at;
    $this->fasilitas = FasilitasKamar::orderBy('nama_fasilitas')->get();
    $this->existingGaleri = $tipeKamar->galeriKamars->map(function($foto) {
        return [
            'id' => $foto->id,
            'url' => $foto->url_foto
        ];
    ...
    // Debug logging after setting state
    \Log::info('State set', [
        'tipeKamar_id' => $this->tipeKamar->id,
        'nama_tipe_state' => $this->nama_tipe,
        'deskripsi_state' => $this->deskripsi,
        'selectedFasilitas_count' => count($this->selectedFasilitas),
        'existing_galeri_count' => count($this->existingGaleri)
    ]);
});
...
$save = function() {
    ...
    try {
        \Log::info('Updating tipe kamar:', [
            'id' => $this->tipeKamar->id,
        ...
        $this->tipeKamar->update([
            'nama_tipe' => $this->nama_tipe,
            'deskripsi' => $this->deskripsi,
        ]);

        \Log::info('TipeKamar updated successfully');

        // Sync fasilitas yang dipilih
        $this->tipeKamar->fasilitasKamars()->sync($this->selectedFasilitas);
        ...
        if (!empty($this->galeriPhotos)) {
            \Log::info('Processing new galeri photos:', ['count' => count($this->galeriPhotos)]);
            ...
            foreach ($this->galeriPhotos as $index => $photo) {
                ...
                GaleriKamar::create([
                    'tipe_kamar_id' => $this->tipeKamar->id,
                    'url_foto' => Storage::url($path),
                ]);
            }
        }

        session()->flash('message', 'Tipe kamar berhasil diperbarui!');
        return $this->redirect(route('admin.tipe-kamar.index'));
    } catch (\Exception $e) {
        \Log::error('Error updating tipe kamar:', [
            'error' => $e->getMessage(),
        ...
        session()->flash('error', 'Gagal memperbarui tipe kamar: ' . $e->getMessage());
    }
};
...
$clearAllPhotos = function() {
    \Log::info('Clearing all new photos');
    $this->galeriPhotos = [];
};

$deleteExistingPhoto = function($photoId) {
    try {
        ...
        // Reload existing galeri
        $this->existingGaleri = $this->tipeKamar->fresh()->galeriKamars->map(function($foto) {
            return [
                'id' => $foto->id,
                'url' => $foto->url_foto
            ];
                ...
                <div
                    ...
                    <form wire:submit="save" class="p-8 space-y-8">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-gray-600 dark:text-gray-400 mb-6">
                                Kelola foto-foto untuk tipe kamar ini
                            </p>
                            ...
                            <div
                                ...
                                <div class="mb-6">
                                    <h4 class="text-sm font-bold text-pink-700 dark:text-pink-300 mb-4">Foto yang sudah
                                        ada:</h4>
                                    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                                        @foreach($existingGaleri as $index => $foto)
                                        <div class="relative group">
                                            ...
                                            <button type="button" wire:click="deleteExistingPhoto({{ $foto['id'] }})"
                                                class="absolute -top-2 -right-2 w-6 h-6 bg-red-500 text-white rounded-full flex items-center justify-center text-xs font-bold hover:bg-red-600 transition-colors duration-300 opacity-0 group-hover:opacity-100">
                                                ×
                                            </button>
                                        </div>
                                        @endforeach
                                ...
                                <div class="mt-4 p-3 bg-gray-100 dark:bg-gray-700 rounded-lg">
                                    <p class="text-xs text-gray-600 dark:text-gray-400">
                                        Debug: Total foto baru di galeriPhotos: {{ count($galeriPhotos) }}
                                    </p>
                                    <p class="text-xs text-gray-600 dark:text-gray-400">
                                        Debug: Total foto baru di incomingPhotos: {{ count($incomingPhotos) }}
                                    </p>
                                    @if(count($galeriPhotos) > 0)
                                    <p class="text-xs text-gray-600 dark:text-gray-400">
                                        Nama file baru: {{ collect($galeriPhotos)->map(fn($p) =>
                                        $p->getClientOriginalName())->implode(', ') }}
                                    </p>
                                    <p class="text-xs text-gray-600 dark:text-gray-400">
                                        Ukuran file baru: {{ collect($galeriPhotos)->map(fn($p) =>
                                ...
                                <div class="mt-6">
                                    <h4 class="text-sm font-bold text-pink-700 dark:text-pink-300 mb-4">
                                        Foto baru yang akan diupload ({{ count($galeriPhotos) }} foto):
                                    </h4>
                                    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                                    ...
                                    <div class="mt-4 flex gap-2">
                                        ...
                                        <span class="text-xs text-gray-500 self-center">
                                            Total ukuran foto baru: {{ number_format(collect($galeriPhotos)->sum(fn($p)
                        ...
                        <div
                            ...
                            <div class="flex items-center mb-4">
                                ...
                                <h4 class="text-sm font-bold text-blue-800 dark:text-blue-300">Informasi Tipe Kamar</h4>
                            </div>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-blue-700 dark:text-blue-400">
                                <div
                                    ...
                                    <p>{{ $created_at ? $created_at->format('d M Y H:i') : 'Tidak tersedia' }}</p>
                                </div>
                                <div
                                    class="bg-white/50 dark:bg-blue-900/30 rounded-xl p-3 border border-blue-200/30 dark:border-blue-700/30">
                                    <p class="font-semibold text-blue-800 dark:text-blue-300 mb-1">Diperbarui</p>
                                    <p>{{ $updated_at ? $updated_at->format('d M Y H:i') : 'Tidak tersedia' }}</p>
                                </div>
                                <div
                                    class="bg-white/50 dark:bg-blue-900/30 rounded-xl p-3 border border-blue-200/30 dark:border-blue-700/30">
                                    <p class="font-semibold text-blue-800 dark:text-blue-300 mb-1">Fasilitas</p>
                                    <p>{{ count($selectedFasilitas) }} fasilitas</p>
                                </div>
                                <div
                                    class="bg-white/50 dark:bg-blue-900/30 rounded-xl p-3 border border-blue-200/30 dark:border-blue-700/30">
                                    <p class="font-semibold text-blue-800 dark:text-blue-300 mb-1">Foto Existing</p>
                                    <p>{{ count($existingGaleri) }} foto</p>
                                </div>
                                <div
                                    class="bg-white/50 dark:bg-blue-900/30 rounded-xl p-3 border border-blue-200/30 dark:border-blue-700/30">
                                    <p class="font-semibold text-blue-800 dark:text-blue-300 mb-1">Foto Baru</p>
                                    <p>{{ count($galeriPhotos) }} foto</p>
                                </div>
                            </div>
                        ...
                        <div
                            ...
                            <button type="submit"
                                ...
                                <span class="relative z-10">Update Tipe Kamar</span>
                            </button>
                        </div>
                    </form>
                </div>
